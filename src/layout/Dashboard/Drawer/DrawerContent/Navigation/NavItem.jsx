...
export default function NavItem({ item, level }) {
  ...
  // 🔹 Debugging: Log item URL to check for undefined values
  console.log("NavItem URL:", item.url);

  // ✅ Fix: Ensure item.url exists before calling matchPath
  const isSelected = item.url 
    ? !!matchPath({ path: item.url, end: false }, pathname) || openItem === item.id 
    : false;

  // Activate menu item on page load
  useEffect(() => {
